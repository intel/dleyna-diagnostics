...
{
	g_variant_builder_add(vb, "{sv}", DLD_INTERFACE_PROP_NEVER_QUIT,
			      g_variant_new_boolean(
					dleyna_settings_is_never_quit(
								settings)));

	g_variant_builder_add(vb, "{sv}", DLD_INTERFACE_PROP_WHITE_LIST_ENABLED,
			      g_variant_new_boolean(
					dleyna_settings_is_white_list_enabled(
								settings)));

	g_variant_builder_add(vb, "{sv}", DLD_INTERFACE_PROP_WHITE_LIST_ENTRIES,
...
static GVariant *prv_get_prop(dleyna_settings_t *settings, const gchar *prop)
{
	...
	if (!strcmp(prop, DLD_INTERFACE_PROP_NEVER_QUIT))
		retval = g_variant_ref_sink(g_variant_new_boolean(
					dleyna_settings_is_never_quit(
								settings)));
	else if (!strcmp(prop, DLD_INTERFACE_PROP_WHITE_LIST_ENABLED))
		retval = g_variant_ref_sink(g_variant_new_boolean(
					dleyna_settings_is_white_list_enabled(
								settings)));
	else if (!strcmp(prop, DLD_INTERFACE_PROP_WHITE_LIST_ENTRIES))
...
static void prv_wl_notify_prop(dld_manager_t *manager,
			       ...
			       GVariant *prop_val)
{
	...
	val = g_variant_new("(s@a{sv}as)", DLEYNA_DIAGNOSTICS_INTERFACE_MANAGER,
			    g_variant_builder_end(&array),
			    NULL);

	(void) dld_diagnostics_get_connector()->notify(
					manager->connection,
...
{
	dld_manager_t *manager = g_new0(dld_manager_t, 1);
			       ...
			       dld_manager_task_complete_t cb)
{
	...
	dld_task_get_props_t *task_data = &task->ut.get_props;
	...
	if (!strcmp(i_name, DLEYNA_DIAGNOSTICS_INTERFACE_MANAGER) ||
	    !strcmp(i_name, "")) {
...
{
	dld_async_task_t *cb_data = (dld_async_task_t *)task;
	dld_task_get_prop_t *task_data = &task->ut.get_prop;
	gchar *i_name = task_data->interface_name;
...
static void prv_set_prop_never_quit(dld_manager_t *manager,
				    dleyna_settings_t *settings,
				    gboolean never_quit,
				    GError **error)
{
...
static void prv_set_prop_wl_enabled(dld_manager_t *manager,
				    dleyna_settings_t *settings,
				    gboolean enabled,
				    GError **error)
{
	...
	if (*error == NULL) {
		...
		prv_wl_notify_prop(manager,
				   DLD_INTERFACE_PROP_WHITE_LIST_ENABLED,
				   prop_val);
	}
...
static void prv_set_prop_wl_entries(dld_manager_t *manager,
				    dleyna_settings_t *settings,
				    GVariant *entries,
				    GError **error)
{
	...
	if (*error == NULL) {
		...
		prv_wl_notify_prop(manager,
				   DLD_INTERFACE_PROP_WHITE_LIST_ENTRIES,
				   entries);
	}
exit:
	DLEYNA_LOG_DEBUG("Exit");
...
{
	dld_async_task_t *cb_data = (dld_async_task_t *)task;
	dld_task_set_prop_t *task_data = &task->ut.set_prop;
	...
	if (!strcmp(name, DLD_INTERFACE_PROP_NEVER_QUIT))
		prv_set_prop_never_quit(manager, settings,
					g_variant_get_boolean(param),
					&error);
	else if (!strcmp(name, DLD_INTERFACE_PROP_WHITE_LIST_ENABLED))
		prv_set_prop_wl_enabled(manager, settings,
					g_variant_get_boolean(param),
					&error);
	else if (!strcmp(name, DLD_INTERFACE_PROP_WHITE_LIST_ENTRIES))
